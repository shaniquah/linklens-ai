...
{
    private BedrockRuntimeClient $bedrockRuntime;
    ...
    public function generateLinkedInPost(array $parameters): string
    {
        ...
        try {
            ...
            return $result['results'][0]['outputText'] ?? 'Failed to generate content';
    ...
    public function analyzeConnectionRequest(array $profileData): array
    {
        ...
        try {
            ...
            return ['decision' => 'reject', 'confidence' => 0.0, 'reason' => 'Analysis failed'];
        }
    }
...
Format: DECISION|CONFIDENCE|REASON";
    ...
    private function parseConnectionDecision(string $analysis): array
    {
        ...
        return [
            'decision' => strtolower(trim($parts[0] ?? 'reject')),
            'confidence' => (float) ($parts[1] ?? 0.0),
